...
def combine_data(
    ...
    for annotation_file in tqdm(
        ...
        for annotation in annotations:
            ...
            if isinstance(annotation['label'], int):
                ...
                raise ValueError(
                    'Unknown label format. Expect either integer or list of '
                    f'floats, but found {annotation["label"].__class__}'
                )
    ...
    with open(file=ground_truth_file, mode='r') as f:
        ground_truth_data = json.load(fp=f)

    gt_annotations = {}
    for gt_annotation in ground_truth_data:
        gt_annotations[gt_annotation['file']] = gt_annotation['label']

    # initialise a list to store all the json data
    # from all the provided datasets
    consolidation = []

    for filename in tqdm(
        iterable=gt_annotations,
        desc='make dataset',
        ncols=80,
        leave=False
    ):
        ...
        # set ground truth
        record['ground_truth'] = gt_annotations.get(filename)
